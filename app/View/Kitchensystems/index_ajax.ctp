	...
	<table class="table table-striped table-bordered  table-hover" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				...
				<th nowrap="">Produits afféctés</th>
		...
		<tbody>
			<?php foreach ($taches as $tache): ?>
				<tr>
					<td nowrap=""><a href="<?php echo $this->Html->url(['action' => 'view', $tache['Kitchensystem']['id']]) ?>"><?php echo h($tache['Kitchensystem']['reference']); ?></a></td>
					<td nowrap=""><a href="<?php echo $this->Html->url(['action' => 'view', $tache['Kitchensystem']['id']]) ?>"><?php echo h($tache['Kitchensystem']['libelle']); ?></a></td>
					<td nowrap=""><?php echo h($tache['Store']['libelle']); ?></td>
					<td nowrap=""><?php echo h($tache['Societe']['designation']); ?></td>
					<td nowrap=""><?php echo h($tache['Kitchensystem']['ip_adresse']); ?></td>
					<td nowrap=""><?php echo count($tache['Produit']); ?> produit(s)</td>
					<td nowrap=""><?php echo h($tache['Kitchensystem']['date_c']); ?></td>
					<td nowrap="" class="actions">
						<a href="<?php echo $this->Html->url(['action' => 'view', $tache['Kitchensystem']['id']]) ?>"><i class="fa fa-eye"></i></a>
						<?php if ($globalPermission['Permission']['m1']): ?>
							<a href="<?php echo $this->Html->url(['action' => 'edit', $tache['Kitchensystem']['id']]) ?>" class="edit"><i class="fa fa-edit"></i></a>
						<?php endif ?>
						<?php if ($globalPermission['Permission']['s']): ?>
							<a href="<?php echo $this->Html->url(['action' => 'delete', $tache['Kitchensystem']['id']]) ?>" class="btnFlagDelete"><i class="fa fa-trash-o"></i></a>
						<?php endif ?>
					</td>
				</tr>
			<?php endforeach; ?>
