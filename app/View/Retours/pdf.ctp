...
<body>
    <div class="container">
        '.$this->element('header',['societe' => $societe,'title' => 'RETOUR']).'
        <table class="info" width="100%">
            <tbody>
                <tr>
                    <td width="50%">RETOUR N° : '.$this->data['Retour']['reference'].'</td>
                    <td width="50%">Date : '.$this->data['Retour']['date'].'</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="info data" width="100%">
            ...
            <tbody>';
                ...
                foreach ($details as $tache){
                    $total = $total + $tache['Retourdetail']['total'];
                    $total_ttc = $total_ttc + $tache['Retourdetail']['ttc'];
                    $total_tva = $total_tva + $tache['Retourdetail']['ttc'] * (1 + (float)$tache['Produit']['tva']/ 100);
                    $html.='<tr>
                        <td width="50%">'.$tache['Produit']['libelle'].'</td>
                        <td width="16%">'.$tache['Retourdetail']['qte'].'</td>
                        <td width="16%" style="text-align:right;">'.number_format($tache['Produit']['prixachat'], 2, ',', ' ').'</td>
                        <td width="16%" style="text-align:right;">'.number_format($tache['Retourdetail']['total'], 2, ',', ' ').'</td>
                    </tr>';
                }
                $html .= '
                    <tr>
...
//echo $html;die;
$pdfName = 'Retour.'.$this->data['Retour']['date'].'.pdf';
$dompdf = new DOMPDF();
