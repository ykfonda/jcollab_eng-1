				...
				<tr>
					<td nowrap="">
						<?php if ( $globalPermission['Permission']['m1'] ): ?>
							<a href="<?php echo $this->Html->url(['action'=>'view',$dossier['Retour']['id']]) ?>"><?php echo h($dossier['Retour']['reference']); ?></a>
						<?php else: ?>
							<?php echo h($dossier['Retour']['reference']); ?>
						<?php endif ?>
					</td>
					...
					<td nowrap=""><?php echo h($dossier['Retour']['date']); ?></td>
					<td nowrap="">
						<?php if ( !empty( $dossier['Retour']['etat'] ) ): ?>
							<span class="badge badge-default" style="width: 100%;background-color: <?php echo $this->App->getEtatRetourColor($dossier['Retour']['etat']); ?>">
								<?php echo $this->App->getEtatRetour($dossier['Retour']['etat']); ?>
							</span>
						<?php endif ?>
					</td>
					<td nowrap="" class="actions">
						<?php if ( $globalPermission['Permission']['m1'] ): ?>
							<a href="<?php echo $this->Html->url(['action'=>'view',$dossier['Retour']['id']]) ?>"><i class="fa fa-eye"></i></a>
						<?php endif ?>
						<?php if ( $globalPermission['Permission']['s'] AND $dossier['Retour']['etat'] == -1 ): ?>
							<a href="<?php echo $this->Html->url(['action' => 'delete', $dossier['Retour']['id']]) ?>" class="btnFlagDelete"><i class="fa fa-trash-o"></i></a>
						<?php endif ?>
					</td>
				</tr>
			<?php endforeach; ?>
