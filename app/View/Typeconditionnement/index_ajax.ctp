	...
	<table class="table table-striped table-bordered  table-hover" cellpadding="0" cellspacing="0">
		...
		<tbody>
			<?php foreach ($taches as $tache): ?>
				<tr>
					<td nowrap=""><?php echo h($tache['Typeconditionnement']['code_type']); ?></td>
					<td nowrap=""><?php echo h($tache['Typeconditionnement']['libelle']); ?></td>
					<td nowrap=""><?php echo h($tache['Typeconditionnement']['date_c']); ?></td>
					<td nowrap="" class="actions">
						<?php if ($globalPermission['Permission']['m1']): ?>
							<a href="<?php echo $this->Html->url(['action' => 'edit', $tache['Typeconditionnement']['id']]) ?>" class="edit"><i class="fa fa-edit"></i></a>
						<?php endif ?>
						<?php if ($globalPermission['Permission']['s'] ): ?>
							<a href="<?php echo $this->Html->url(['action' => 'delete', $tache['Typeconditionnement']['id']]) ?>" class="btnFlagDelete"><i class="fa fa-trash-o"></i></a>
						<?php endif ?>
					</td>
				</tr>
			<?php endforeach; ?>
