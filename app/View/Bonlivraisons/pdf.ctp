...
<html>
<head>
    <title>VENTE N° : '.$this->data['Bonlivraison']['reference'].'</title>
    '.$this->element('style').'
</head>
<body>
    ...
    position: relative;
    left: -9.1rem;font-size : 110%">Bonlivraison</caption>
    <thead>
    <tr>
      
      <th scope="col" style="width : 6rem">Date</th>
      <th scope="col" style="width : 9rem">Bonlivraison N°</th>
    </tr>
  </thead>
  <tbody>
    <tr>
     
      <td scope="row">'.date("d-m-Y",strtotime($this->data['Bonlivraison']['date'])).'</td>
      <td>'.$this->data['Bonlivraison']['reference'].'</td>
    </tr>
    </tbody>
    <div class="rounded">
            ...
            <tbody>';
                foreach ($details as $tache){
                    $html.='<tr>
                        ...
                        <td nowrap style="text-align:right;">'.number_format($tache['Bonlivraisondetail']['qte'], 2, ',', ' ').'</td>
                ...
                <div style="position : relative; float : left; margin-left : 0.9rem">
                        ...
                        <p style="width:50%;text-align:left">
                            '.strtoupper( $this->Lettre->NumberToLetter( strval($this->data['Bonlivraison']['total_apres_reduction']) ) ).'
                        </p>
                        </div>  
                ...
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonlivraison']['montant_tva'], 2, ',', ' ').'</p> <br><br>
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL REMISE('.$this->data['Bonlivraison']['remise'].'%) : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonlivraison']['montant_remise'], 2, ',', ' ').'</p><br><br>
           
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL TTC : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonlivraison']['total_apres_reduction'], 2, ',', ' ').'</p><br><br>
...
//echo $html;die;
$pdfName = 'Bonlivraison.'.$this->data['Bonlivraison']['date'].'.pdf';
$dompdf = new DOMPDF();
