...
<html>
<head>
    <title>VENTE N° : '.$this->data['Boncommande']['reference'].'</title>
    '.$this->element('style').'
</head>
<body>
    ...
    position: relative;
    left: -8.6rem;font-size : 110%">Boncommande</caption>
    <thead>
    <tr>
      
      <th scope="col" style="width : 6rem">Date</th>
      <th scope="col" style="width : 10rem">Boncommande N°</th>
    </tr>
  </thead>
  <tbody>
    <tr>
     
      <td scope="row">'.date("d-m-Y",strtotime($this->data['Boncommande']['date'])).'</td>
      <td>'.$this->data['Boncommande']['reference'].'</td>
    </tr>
    </tbody>
    <div class="rounded">
            ...
            <tbody>';
                foreach ($details as $tache){
                    $html.='<tr>
                        ...
                        <td nowrap style="text-align:right;">'.number_format($tache['Boncommandedetail']['qte'], 2, ',', ' ').'</td>
                        <td nowrap style="text-align:right;">'.number_format($tache['Boncommandedetail']['prix_vente'], 2, ',', ' ').'</td>
                        <td nowrap style="text-align:right;">'.(int)$tache['Boncommandedetail']['remise'].'%</td>
                ...
                <div style="position : relative; float : left; margin-left : 0.9rem">
                        ...
                        <p style="width:50%;text-align:left">
                            '.strtoupper( $this->Lettre->NumberToLetter( strval($this->data['Boncommande']['total_apres_reduction']) ) ).'
                        </p>
                        </div>  
                ...
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Boncommande']['montant_tva'], 2, ',', ' ').'</p> <br><br>
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL REMISE('.$this->data['Boncommande']['remise'].'%) : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Boncommande']['remise'], 2, ',', ' ').'</p><br><br>
           
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL TTC : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Boncommande']['total_apres_reduction'], 2, ',', ' ').'</p><br><br>
...
//echo $html;die;
$pdfName = 'Boncommande.'.$this->data['Boncommande']['date'].'.pdf';
$dompdf = new DOMPDF();
