...
<html>
<head>
    <title>VENTE N° : '.$this->data['Bonavoir']['reference'].'</title>
    '.$this->element('style').'
</head>
<body>
    ...
    position: relative;
    left: -9.9rem;font-size : 110%">Bonavoir</caption>
    <thead>
    <tr>
      
      <th scope="col" style="width : 6rem">Date</th>
      <th scope="col" style="width : 8rem">Bonavoir N°</th>
    </tr>
  </thead>
  <tbody>
    <tr>
     
      <td scope="row">'.date("d-m-Y",strtotime($this->data['Bonavoir']['date'])).'</td>
      <td>'.$this->data['Bonavoir']['reference'].'</td>
    </tr>
    </tbody>
    <div class="rounded">
            ...
            <tbody>';
                foreach ($details as $tache){
                    $html.='<tr>
                        ...
                        <td nowrap style="text-align:right;">'.number_format($tache['Bonavoirdetail']['qte'], 2, ',', ' ').'</td>
                        <td nowrap style="text-align:right;">'.number_format($tache['Bonavoirdetail']['prix_vente'], 2, ',', ' ').'</td>
                        <td nowrap style="text-align:right;">'.(int)$tache['Bonavoirdetail']['remise'].'%</td>
                ...
                <div style="position : relative; float : left; margin-left : 0.9rem">
                        ...
                        <p style="width:50%;text-align:left">
                            '.strtoupper( $this->Lettre->NumberToLetter( strval($this->data['Bonavoir']['total_apres_reduction']) ) ).'
                        </p>
                        </div>  
                ...
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonavoir']['montant_tva'], 2, ',', ' ').'</p> <br><br>
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL REMISE('.$this->data['Bonavoir']['remise'].'%) : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonavoir']['remise'], 2, ',', ' ').'</p><br><br>
           
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL TTC : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Bonavoir']['total_apres_reduction'], 2, ',', ' ').'</p><br><br>
...
//echo $html;die;
$pdfName = 'Bonavoir.'.$this->data['Bonavoir']['date'].'.pdf';
$dompdf = new DOMPDF();
