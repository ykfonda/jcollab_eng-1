...
$objPHPExcel->getProperties()->setCreator("WEB-CRM")->setLastModifiedBy("WEB-CRM")->setTitle("CreditFournisseur");
...
$objPHPExcel->getActiveSheet()->setTitle('CreditFournisseur');
...
for ($i = 0; $i < count($taches); $i++){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue('A'.$cell, $taches[$i]['Fournisseur']['reference'])
	            ->setCellValue('B'.$cell, $taches[$i]['Fournisseur']['designation'])
...
header('Content-Disposition: attachment;filename="CreditFournisseur.xls"');
