...
<body>
    <div class="container">
        '.$this->element('header',['societe' => $societe,'title' => 'DECHARCHEMENT']).'
        <table class="info" width="100%">
            <tbody>
                <tr>
                    <td width="50%">DECHARCHEMENT N° : '.$this->data['Decharchement']['reference'].'</td>
                    <td width="50%">Date : '.$this->data['Decharchement']['date'].'</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="info data" width="100%">
            ...
            <tbody>';
                ...
                foreach ($details as $tache){
                    $total = $total + $tache['Decharchementdetail']['total'];
                    $total_ttc = $total_ttc + $tache['Decharchementdetail']['ttc'];
                    $total_tva = $total_tva + $tache['Decharchementdetail']['ttc'] * (1 + (float)$tache['Produit']['tva']/ 100);
                    $html.='<tr>
                        <td width="50%">'.$tache['Produit']['libelle'].'</td>
                        <td width="16%">'.$tache['Decharchementdetail']['qte'].'</td>
                        <td width="16%" style="text-align:right;">'.number_format($tache['Produit']['prixachat'], 2, ',', ' ').'</td>
                        <td width="16%" style="text-align:right;">'.number_format($tache['Decharchementdetail']['total'], 2, ',', ' ').'</td>
                    </tr>';
                }
                $html .= '
                    <tr>
...
//echo $html;die;
$pdfName = 'Decharchement.'.$this->data['Decharchement']['date'].'.pdf';
$dompdf = new DOMPDF();
