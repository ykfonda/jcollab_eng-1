  ...
  <div class="portlet-title">
    ...
    <div class="actions">
      ...
      <?php if ($globalPermission['Permission']['a']): ?>
        ...
        <a href="<?php echo $this->Html->url(['action' => 'edit']) ?>" class="edit btn btn-primary btn-sm"><i class="fa fa-plus"></i> Nouveau produit </a>
      <?php endif ?>
    </div>
  </div>
...
<script>
  ...
  $('#edit').on('change','#ProduitCategorieproduitId',function(e){
  ...
  function completeFn(){
    ...
    if(table.length > 0 && safe == true){   
      ...
      $("#pese").empty();
      $("#pese").append("<option value=''>--Vide</option>");
      for (var i = 0; i <= table.length; i++) {
          if ( table[i] != 'undefined' && table[i] != '') {
            $("#pese").append("<option value='"+i+"'>"+table[i]+"</option>");
        }
      }
      $("#pese").val(7);

      $("#cpt_achat").empty();
      $("#cpt_achat").append("<option value=''>--Vide</option>");
      for (var i = 0; i <= table.length; i++) {
          if ( table[i] != 'undefined' && table[i] != '') {
            $("#cpt_achat").append("<option value='"+i+"'>"+table[i]+"</option>");
        }
      }
      $("#cpt_achat").val(8);

      $("#cpt_vente").empty();
      $("#cpt_vente").append("<option value=''>--Vide</option>");
      for (var i = 0; i <= table.length; i++) {
          if ( table[i] != 'undefined' && table[i] != '') {
            $("#cpt_vente").append("<option value='"+i+"'>"+table[i]+"</option>");
        }
      }
      $("#cpt_vente").val(9);
