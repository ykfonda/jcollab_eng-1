...
<html>
<head>
    <title>VENTE N° : '.$this->data['Facture']['reference'].'</title>
    '.$this->element('style').'
</head>
<body>
    ...
    <tr>
      ...
      <th scope="col" style="width : 6rem">Facture N°</th>
    </tr>
  </thead>
  <tbody>
    <tr>
     
      <td scope="row">'.date("d-m-Y",strtotime($this->data['Facture']['date'])).'</td>
      <td>'.$this->data['Facture']['reference'].'</td>
    </tr>
    </tbody>
    <div class="rounded">
            ...
            <tbody>';
                foreach ($details as $tache){
                    $html.='<tr>
                        ...
                        <td nowrap style="text-align:right;">'.number_format($tache['Facturedetail']['qte'], 2, ',', ' ').'</td>
                ...
                <div style="position : relative; float : left; margin-left : 0.9rem">
                        ...
                        <p style="width:50%;text-align:left">
                            '.strtoupper( $this->Lettre->NumberToLetter( strval($this->data['Facture']['total_apres_reduction']) ) ).'
                        </p>
                        </div>  
                ...
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Facture']['montant_tva'], 2, ',', ' ').'</p> <br><br>
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL REMISE('.$this->data['Facture']['remise'].'%) : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Facture']['montant_remise'], 2, ',', ' ').'</p><br><br>
           
                <p style="display : inline;font-weight : bold;color : #282525;text-decoration : underline;font-size : 95%">TOTAL TTC : </p>
                <p style="display : inline;font-size : 95%">'.number_format($this->data['Facture']['total_apres_reduction'], 2, ',', ' ').'</p><br><br>
...
//echo $html;die;
$pdfName = 'Facture.'.$this->data['Facture']['date'].'.pdf';
$dompdf = new DOMPDF();
