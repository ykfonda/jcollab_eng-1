...
<script type="text/javascript">
	var dataFilter = null;
  	var dataPage = 1;
	  var formtest = "<?php echo $form; ?>"; 

	Init();
	
	$('#Brt09Form').on('submit',function(e){
    $('.saveBtn').attr('disabled',true);
  });
	<?php if ($ajax): ?>
	  	$('#edit').on('submit','<?php echo $form; ?>',function(e){
		
		if(formtest == "#FactureEditForm" ) $(".#FactureEditForm").submit();
		if(formtest == "#BonretourEditForm" ) $(".#BonretourEditForm").submit();
		if(formtest == "#Brt09Form" ) $("#Brt09Form").submit();

		if( formtest != "#BonretourachatEditForm" )		
	      e.preventDefault();
