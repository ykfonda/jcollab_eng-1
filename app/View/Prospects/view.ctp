	    ...
	    <div class="row">
	      	<div class="col-md-12">
		          ...
		          <table class="table table-bordered tableHeadInformation">
		            <tbody>
		              <tr>
		                <td class="tableHead" nowrap="">Désignation - Référence</td>
		                <td nowrap="">
		                  <?php echo $this->data['Client']['designation'] ?> <?php echo $this->data['Client']['reference'] ?>
		                </td>
...
<div class="portlet light bordered">
	<div class="portlet-title">
		...
		<div class="actions">
			<?php if ($globalPermission['Permission']['m1']): ?>
				<a href="<?php echo $this->Html->url(['action' => 'changelocation',$this->data['Client']['id']]) ?>" class="changeLocation btn btn-primary btn-sm"><i class="fa fa-map-marker"></i> Actualisé la position prospect </a>
			<?php endif ?>
		</div>
	</div>
	<div class="portlet-body">
...
<script>
	$(function(){
		...
		$('.changeLocation').on('click',function(e){
		    ...
		    bootbox.confirm("Êtes-vous sûr de vouloir changer la géolocalisation de ce prospect ?<br/><strong>Attention : </strong> la géolocalisation de ce prospect va changer par votre position actuelle ! ", function(result) {
		      if( result ){
				getLocation(url);
		      }
		    });
