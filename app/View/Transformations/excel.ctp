...
$objPHPExcel->getProperties()->setCreator("JCOLLAB")->setLastModifiedBy("JCOLLAB")->setTitle("Transformations");
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F'.$headTable, 'Statut')
            ->setCellValue('G'.$headTable, 'Date création');

$objPHPExcel->getActiveSheet()->getStyle('A'.$headTable.':I'.$headTable)->applyFromArray($styleBorder);

$objPHPExcel->getActiveSheet()->setTitle('Transformations');
...
for ($i = 0; $i < count($taches); $i++){
	$cell = $headTable + $i + 1;
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue('A'.$cell, $taches[$i]['Transformation']['reference'])
	            ->setCellValue('B'.$cell, $taches[$i]['Transformation']['libelle'])
	            ->setCellValue('C'.$cell, $taches[$i]['Transformation']['date'])
	            ->setCellValue('D'.$cell, $taches[$i]['User']['nom'].' '.$taches[$i]['User']['prenom'])
                ->setCellValue('E'.$cell, $taches[$i]['Depot']['libelle'])
                ->setCellValue('F'.$cell, $this->App->getValideEntree($taches[$i]['Transformation']['statut']) )
                ->setCellValue('G'.$cell, $taches[$i]['Transformation']['date_c']);
    $objPHPExcel->getActiveSheet()->getStyle('A'.$cell.':G'.$cell)->applyFromArray($styleBorder);
}
//die;
// Redirect output to a client’s web browser (Excel5)
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Transformations.xls"');
