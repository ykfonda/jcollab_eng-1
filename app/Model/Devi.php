...
class Devi extends AppModel
{
    ...
    public $hasMany = ['Devidetail', 'Avance', 'Email'];
    ...
    public function beforeSave($options = [])
    {
        ...
        if (empty($this->data[$this->alias]['id']) and empty($this->data[$this->alias]['reference'])) {
            ...
            $options = ['conditions' => ['module' => 'Devis', 'store_id' => CakeSession::read('Auth.User.StoreSession.id')]];
            ...
            $conditions['Devi.reference LIKE '] = "%$prefix%";
            $commmande = $this->find('first', ['conditions' => $conditions, 'order' => ['id' => 'DESC']]);
            if (isset($commmande['Devi']['id'])) {
                $str = explode('-', $commmande['Devi']['reference']);
