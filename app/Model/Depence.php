<?php 
class Depence extends AppModel
{
	public $belongsTo = [
		'Boncommande' => array(
		...
		'Categoriedepence' => array(
			'className' => 'Categoriedepence',
			'foreignKey' => 'categoriedepence_id',
		),
		'Creator' => [
			'className' => 'User',
            'foreignKey' => 'user_c'
	...
	public function beforeSave($options = array()){
		...
		if ( empty($this->data[$this->alias]['id']) AND empty( $this->data[$this->alias]['reference'] ) ){ 
			$number = $this->find('count',['conditions'=>[$this->alias.'.deleted'=>[0,1]]]) + 1; $this->data[$this->alias]['reference'] = 'DP-'.str_pad($number, 6, '0', STR_PAD_LEFT);
		}
