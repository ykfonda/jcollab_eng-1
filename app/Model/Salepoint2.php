...
class Salepoint2 extends AppModel
{
    ...
    public function paginate($conditions, $fields, $order, $limit, $page=1, $recursive=null, $extra=array())
    {
      ...
      if (!empty($date1) and !empty($date2) and !empty($magasin_id)) {
        ...
        stores store where store.id = s.store) as magasin,  Sum(total_a_payer_ht + fee / 1.2) as Totalht,Sum(total_apres_reduction - montant_remise) as Totalttc, SUM((Select Sum(a.montant) from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= '$date1') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1'  
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= '$date1') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1'  GROUP BY s.store  LIMIT $row_start, $limit ");
        }
        else if(empty($date1) and empty($date2) and !empty($magasin_id)) {
            return $this->query("Select (Select store.libelle  from
        stores store where store.id = s.store) as magasin,  Sum(total_a_payer_ht + fee / 1.2) as Totalht,Sum(total_apres_reduction - montant_remise) as Totalttc, SUM((Select Sum(a.montant) from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  and s.store = $magasin_id  GROUP BY s.store LIMIT $row_start, $limit ");
        }
      ...
      avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
      avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
      salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and and s4.date >= DATE( NOW()) and s4.date <= concat(DATE( NOW()),' ','23:59:59')) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= concat(DATE( NOW()),' ','00:00:00') and s.date <= concat(DATE( NOW()),' ','23:59:59')  GROUP BY s.store 
        LIMIT $row_start, $limit
      ");
