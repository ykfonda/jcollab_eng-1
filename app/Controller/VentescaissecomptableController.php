...
class VentescaissecomptableController extends AppController
{
    public $uses = ["Salepoint33"];
    public $idModule = 123;
 ...
 */
        ...
        foreach ($this->params['named'] as $param_name => $value) {
                 ...
                 if ($param_name == 'Salepoint3.magasin_id') {
                    /* $this->loadModel("Store");
 ...
 */                   $arr["magasin_id"] = $value;
       ...
       // var_dump($arr["date1"],$arr["date2"],$arr["magasin_id"]);
           $counts = $this->Salepoint33->query("Select (Select store.libelle  from
           stores store where store.id = s.store) as magasin, date(s.date) as datec ,(Select caisse.libelle  from
           ...
           users user where user.id = s.user_id) as prenom,  Sum(total_apres_reduction - montant_remise) as Totalttc, SUM((Select Sum(a.montant) from
           ...
           salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id and s4.date >= DATE( NOW()) and s4.date <= concat(DATE( NOW()),' ','23:59:59') ) as mntAnnule FROM salepoints s 
           where s.store != 0 and s.etat = 2 and s.date >= DATE( NOW()) and s.date <= concat(DATE( NOW()),' ','23:59:59') GROUP BY s.store,s.caisse_id,datec
      ...
      /*   $selected_store  = $this->Session->read('Auth.User.StoreSession.id');
        ...
        $conditions['Salepoint.store'] = $selected_store; */
         if(!empty($date1) and !empty($date2) and !empty($magasin_id)) {
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id  and s4.date >= '$date1' and s4.date <= '$date2' ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  
            and s.reference NOT LIKE '%T09%' and s.date >= '$date1' and s.date <= '$date2' 
            and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, datec ");
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id and s4.date >= '$date1' and s4.date <= '$date2' ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.reference NOT LIKE '%T09%' and s.date >= '$date1' and s.date <= '$date2' 
          GROUP BY s.store,s.caisse_id,datec  ");
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id and s4.date >= '$date1') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.reference NOT LIKE '%T09%' and s.date >= '$date1'  
            and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, datec ");
        }
        else if(empty($date1) and !empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint33->query("Select (Select store.libelle  from
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id  and s4.date <= '$date2') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.reference NOT LIKE '%T09%' and s.date <= '$date2' 
            and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, datec ");
        }
        else if(empty($date1) and !empty($date2) and empty($magasin_id)) {
            $counts = $this->Salepoint33->query("Select (Select store.libelle  from
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id  and s4.date >= '$date1' ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.reference NOT LIKE '%T09%' and s.date >= '$date1'  GROUP BY s.store,s.caisse_id, datec ");
        }
        else if(empty($date1) and empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint33->query("Select (Select store.libelle  from
            stores store where store.id = s.store) as magasin, date(s.date) as datec ,(Select caisse.libelle  from
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit,Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  and s.reference NOT LIKE '%T09%' and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, datec ");
        }
           ...
           $totalttc = 0;$especes = 0;$T09 = 0;$Ecommerce = 0;$Wallet = 0;$Carte = 0;
			$Cheque = 0;$Bon_achat = 0;$cheque_cadeau = 0;$offert = 0;$virement = 0;$Cmi = 0;
			$Credit = 0;$remise = 0;$mntAnnule = 0;$totall = []; 
            ...
            foreach ($counts as $dossier) {
                ...
                $mntAnnule += !empty($dossier[0]['mntAnnule']) ? floatval($dossier[0]['mntAnnule']) : 0;
        ...
        'Ecommerce','Wallet','Carte','Cheque','Bon_achat','cheque_cadeau',
          'offert','virement','Credit','remise','mntAnnule'));
