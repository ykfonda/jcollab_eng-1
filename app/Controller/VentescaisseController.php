...
class VentescaisseController extends AppController
{
    public $uses = ["Salepoint3"];
    public $idModule = 123;
 ...
 */                   $arr["magasin_id"] = $value;
       ...
       $counts = $this->Salepoint3->query("Select (Select store.libelle  from
      ...
      /*   $selected_store  = $this->Session->read('Auth.User.StoreSession.id');
        ...
        $conditions['Salepoint.store'] = $selected_store; */
         if(!empty($date1) and !empty($date2) and !empty($magasin_id)) {
            ...
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id and s4.date >= '$date1' and s4.date <= '$date2') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  
              and s.date >= '$date1' and s.date <= '$date2' 
        and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, s.user_id, datec");
          ...
          GROUP BY s.store,s.caisse_id, s.user_id, datec ");
        }
        else if(!empty($date1) and empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint3->query("Select (Select store.libelle  from
            stores store where store.id = s.store) as magasin,(Select caisse.libelle  from
        ...
        and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, s.user_id, datec ");
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id and s4.date >= '$date1') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1'  GROUP BY s.store,s.caisse_id, s.user_id, datec ");
        }
        else if(empty($date1) and empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint3->query("Select (Select store.libelle  from
            stores store where store.id = s.store) as magasin,(Select caisse.libelle  from
            ...
            avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
            avances a where a.salepoint_id = s.id and a.mode ='delayed' )) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
            salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.caisse_id = s.caisse_id ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  and s.store = $magasin_id  GROUP BY s.store,s.caisse_id, s.user_id, datec ");
        }
           ...
           $totalttc = 0;$especes = 0;$T09 = 0;$Ecommerce = 0;$Wallet = 0;$Carte = 0;
			...
			$tpe = 0;$remise = 0;$mntAnnule = 0;$totall = []; 
            ...
            foreach ($counts as $dossier) {
                ...
                $totall[] = $totalttc +$especes +$T09 +$Ecommerce +$Wallet +$Carte +
                $Cheque +$Bon_achat +$cheque_cadeau +$offert +$virement +$Credit;
