...
class PosController extends AppController
{
    //public $idModule = 120;
    public $idModule = 130;
    public $uses = ['Produit', 'Role', 'Commandeglovo', 'Remiseclient', 'User', 'Salepoint', 'Sessionuser', 'Caisse', 'Attente', 'Commande', 'Parametreste', 'Ecommerce', 'Optionproduit', 'Typeconditionnementtproduit', 'Store', 'Salepointdetail'];
    public $notice = 0;
    public $created_at;
        ...
        if (empty($salepoint_id)) {
            ...
            if (($session_cloture == 1) or ($session_nv == 1 and $session_en_cours == false)) {
                ...
                $data = [
                        ...
                        'user_id' => $user_id,
                        'site_id' => $this->Session->read('Auth.User.StoreSession.id'),
...
*/
                ...
                */     //$reste_a_payer = $salepoint['Salepoint']['total_paye'] + $salepoint['Salepoint']['fee'] - $total_paye;
                ...
                //sortie de la qunatite

                App::import('Controller', 'Bonretourachats');
                $Bonretour = new BonretourachatsController();
                foreach ($details as $key => $value) {
                    $Bonretour->sortie($value['Salepointdetail']['produit_id'], $salepoint['Salepoint']['depot_id'], 0, $value['Salepointdetail']['qte'], 0);
                }
 ...
 */
    ...
    public function syncmanuel($store_id = null)
    {


        $this->loadModel('Salepointdetail');
        $salepointdetail = new Salepointdetail();
...
$data_ok = array(
    ...
    'salepoint_id' => 3,
    'commandedetail_id' => 4,
    'ecommercedetail_id' => 5,
    'categorieproduit_id' => 6,
    'qte_cmd' => 10,
    'qte' => 8,
    'prix_vente' => 15.99,
    'unit_price' => 12.99,
    ...
    'glovodetail_id' => 7
        ...
        $salepoints_ready_togo = $this->Salepoint->find('all', [
            'contain' => ['Salepointdetail'=>[
                'conditions' => ['Salepointdetail.deleted'=>0],
            ]],
            'conditions' => [
    ...
    $this->Salepoint->Salepointdetail->setDataSource('sync_server');

     if ($store_id == 6789) {
        //if ($this->Salepoint->saveAll($salepoints_ready_togo)) {
       // if ($this->Salepoint->Salepointdetail->saveAll($details_ready_togo)) {
           // Afficher le message
           echo "DATA HAS BEEN SENT! <br />";
           $this->Session->setFlash('DATA HAS BEEN SENT!', 'alert-success');

           // Mettre à jour l'etat de sync = 1
               // Retour à la première base de données
               $this->Salepoint->setDataSource('default');
               // Requete et conditions
               $this->Salepoint->updateAll(
               ['Salepoint.sync' => 1], // tableau des champs à mettre à jour : 1  Synchronisé 
               ['Salepoint.store' => $store_id, 'Salepoint.caisse_id' => $caisse_id, 'Salepoint.sync' => 0] // tableau des conditions
           );
       }
    ...
    // FOR TESTE
    public function getsalestosync()
    {
