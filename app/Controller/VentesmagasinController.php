...
class VentesmagasinController extends AppController
{
    public $uses = ["Salepoint2"];
    public $idModule = 123;
 ...
 */                   $arr["magasin_id"] = $value;
       ...
       $counts = $this->Salepoint2->query("Select (Select store.libelle  from
        stores store where store.id = s.store) as magasin,  Sum(total_a_payer_ht + s.fee / 1.20) as Totalht,Sum(total_apres_reduction - montant_remise) as Totalttc, SUM((Select Sum(a.montant) from
        ...
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= concat(DATE( NOW()),' ','00:00:00') and s4.date <= concat(DATE( NOW()),' ','23:59:59') ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= concat(DATE( NOW()),' ','00:00:00') and s.date <= concat(DATE( NOW()),' ','23:59:59') 
         GROUP BY s.store 
      ...
      /*   $selected_store  = $this->Session->read('Auth.User.StoreSession.id');
       ...
       avances a where a.salepoint_id = s.id and (a.mode ='Carte' or a.mode ='cmi' or a.mode ='tpe'))) as Carte, SUM((Select Sum(a.montant) from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed')) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= '$date1' and s4.date <= '$date2') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1' and s.date <= '$date2' 
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed')) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= '$date1' and s4.date <= '$date2') as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1'  
        and s.store = $magasin_id  GROUP BY s.store  ");
        }
        else if(empty($date1) and !empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint2->query("Select (Select store.libelle  from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed')) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date <= '$date2' ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date <= '$date2' 
        and s.store = $magasin_id  GROUP BY s.store  ");
        }
        else if(empty($date1) and !empty($date2) and empty($magasin_id)) {
            $counts = $this->Salepoint2->query("Select (Select store.libelle  from
            ...
            $counts = $this->Salepoint2->query("Select (Select store.libelle  from
        stores store where store.id = s.store) as magasin,  Sum(total_a_payer_ht + s.fee / 1.20) as Totalht,Sum(total_apres_reduction - montant_remise)  as Totalttc, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and and s.reference NOT LIKE '%T09%'  (a.mode ='cod' or a.mode = 'espece'))) as Especes, Sum((Select s2.total_apres_reduction from
        salepoints s2 where s2.id = s.id and s2.reference LIKE '%T09%' )) as T09,Sum((Select s3.total_apres_reduction from
        salepoints s3 where s3.id = s.id and s3.ecommerce_id != 0 )) as Ecommerce, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='wallet' )) as Wallet, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and (a.mode ='Carte' or a.mode ='cmi' or a.mode ='tpe'))) as Carte, SUM((Select Sum(a.montant) from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed')) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store and s4.date >= '$date1' ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2 and s.date >= '$date1'  GROUP BY s.store  ");
        }
        else if(empty($date1) and empty($date2) and !empty($magasin_id)) {
            $counts = $this->Salepoint2->query("Select (Select store.libelle  from
        stores store where store.id = s.store) as magasin,  Sum(total_a_payer_ht + s.fee / 1.20) as Totalht, Sum(total_apres_reduction - montant_remise)  as Totalttc, SUM((Select Sum(a.montant) from
        ...
        avances a where a.salepoint_id = s.id and a.mode ='virement' )) as virement, SUM((Select Sum(a.montant) from
        avances a where a.salepoint_id = s.id and a.mode ='delayed')) as Credit, Sum(remise) as remise,(Select SUM(s4.total_apres_reduction) from
        salepoints s4 where  s4.etat = 3 and s4.reference not like '%PROV%' and s4.store = s.store ) as mntAnnule FROM salepoints s where s.store != 0 and s.etat = 2  and s.store = $magasin_id  GROUP BY s.store  ");
        }
          ...
          'T09','Ecommerce','Wallet','Carte','Cheque','Bon_achat','cheque_cadeau',
