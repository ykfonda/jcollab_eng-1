...
class PosController extends AppController
{
    //public $idModule = 120;
    public $idModule = 130;
    public $uses = ['Produit', 'Role', 'Commandeglovo', 'Remiseclient', 'User', 'Salepoint', 'Sessionuser', 'Caisse', 'Attente', 'Commande', 'Parametreste', 'Ecommerce', 'Optionproduit', 'Typeconditionnementtproduit', 'Store', 'Salepointdetail','Ecommercedetail' ,'Client',];
    public $notice = 0;
    public $created_at;

    public function beforeFilter()
    {
        $this->Auth->allow('syncmanuel');
      //  parent::beforeFilter();
    }
    ...
    // Ajouter un loading de la page avant d'afficher le contenu de POS
       ...
       // $get_clients = $this->requestAction(array('controller' => 'Clients', 'action' => 'insertClientsApi'));
        // 3 - Sync users
       // $get_clients = $this->requestAction(array('controller' => 'Users', 'action' => 'insertUsersApi'));
 ...
 */
    ...
    public function ecommerce($salepoint_id = null, $barcode = null)
    {
        ...
        // appel fonction de sync des ECOM caisse => serveur
        $this->syncmanuelecom();
           ...
           // tableau des champs à mettre à jour : 1  Synchronisé 
            if ( $this->Salepoint->updateAll(['Salepoint.sync' => 1], ['Salepoint.id' => $salepoint_ids]) ) {
                ...
                // Si les ventes sonts Synchronisé : 
                    ...
                    // ça marche mais il faut vérifier user_c ****************************
                    //$this->loadModel('Synchronisation');
                    //$this->Synchronisation->save(array(
                    //'source' => $server_link,
                    //'module' => "Vente POS",
                    //'user_c' => 0, // le déclencheur
        ...
        public function syncmanuelecom($store_id = null)
        {
            ...
            $salepoints_ready_togo = $this->Ecommerce->find('all',[
                ...
                'conditions' => [
                    ...
                    'Ecommerce.etat' => 2, // que les etat traité
                ]
            ]);
