...
class BonreceptionsController extends AppController {
	...
	public function index() {
		...
		// $depots = $this->Bonreception->Depot->findList(['Depot.id'=>$selected_store]);
	...
	public function scan($code_barre = null, $depot_id = null) {
		...
		if ( $longeur != 13 ) $response['message'] = "Code a barre est incorrect , produit introuvable !";
			...
			if ( $cb_identifiant != $identifiant ) $response['message'] = "Identifiant du code à barre est incorrect , veuillez vérifier votre paramétrage d'application !";
				...
				$produit = $this->Bonreception->Bonreceptiondetail->Produit->find('first',['fields'=>['id','prix_vente','stockactuel','unite_id','prixachat'],'conditions' => [ 'Produit.type' => 2,'Produit.code_barre' => $code_article ]]);
				if( !isset($produit['Produit']['id']) ) {
					$response['message'] = "Code a barre incorrect produit introuvable !";
				}
				
				$produit = $this->Bonreception->Bonreceptiondetail->Produit->find('first',[
	...
	public function indexAjax(){
		...
		$role_id = $this->Session->read('Auth.User.role_id');
		$conditions = [];
	/* 	if ( !in_array($role_id, $admins) ) $conditions['Bonreception.user_c'] = $user_id; */
	...
	//$depot_dest = $this->Bonreception->Depot->find("first",["conditions" => ["Depot.id" => $this->request->data["Bonreception"]["depot_id"]]]);
	...
	public function view($id = null) {
       ...
       /*  if ($this->request->data["Bonreception"]["type"] == "Expedition") {
            $depot_src = $this->Bonreception->Depot->find('first', ["conditions" => ["id" => $this->request->data["Bonreception"]["depot_id"], "deleted" => 0]]);
		...
		if ($this->Bonreception->saveField('etat',$etat)) {
			...
			if ( isset( $boncommande['Bonreception']['id'] ) AND $boncommande['Bonreception']['etat'] == 2 ) {
				...
				foreach ($details as $key => $value) {
					...
					$produit_id_tosave 			= $value['Bonreceptiondetail']['produit_id'];
					$depot_id_tosave			= $value['Bonreceptiondetail']['depot_id'];
					$quantite_entree_tosave 	= $value['Bonreceptiondetail']['qte'];
					...
					$this->entree($value['Bonreceptiondetail']['produit_id'],$depot_id,$value['Bonreceptiondetail']['qte'],$value['Bonreceptiondetail']['prix_vente']);
				}
			}
			$this->Session->setFlash("L'enregistrement a été effectué avec succès.",'alert-success');
		} else {
